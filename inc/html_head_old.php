...
function html_head(
    ...
    // Check for authentication routes dynamically
    if (in_array('auth', $segments)) {
        if (in_array('login', $segments) || in_array('start-application', $segments) || in_array('forgot-password', $segments)) {
            $marginTop = '0px'; // No margin for authentication pages
        }
    }

    // Properly apply margin style
    ...
    // Prepare SEO and Schema content
    $seoContent = $includeSeoContent ? include($includeSeoContent) : '';
    $schemaContent = $includeSchema ? include($includeSchema) : '';
