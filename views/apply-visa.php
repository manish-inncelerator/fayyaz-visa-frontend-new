<?php
('BASE_DIR') || die('Direct access denied');
...
<?php
function extractCountryFromSlug($slug)
{
    $original = strtolower($slug);

    // Clean up the slug
    $slug = preg_replace([
        '/^apply-for-/',                  // remove prefix
        '/\.html$/',                      // remove .html
        '/\/$/',                          // remove trailing slash
        '/\((.*?)\)/',                    // remove anything in brackets
        '/[^a-z\-]/',                     // remove non-letter characters except dash
    ...
    // Loop through parts and ignore non-country related words
    foreach ($parts as $part) {
        // Skip common non-country words
        if (in_array($part, ['visa', 'online', 'evisa', 'schengen', 'apply', 'for'])) {
            continue;
        }

        // Capitalise each part
        $countryParts[] = ucfirst($part);
    }

    return implode(' ', $countryParts);
}
...
function countryToEmoji($countryName)
{
    ...
    $countries = [
        ...
        "Zambia" => "ZM",
        "Zimbabwe" => "ZW",
        // Added territories and regions with emoji-flag support
        "American Samoa" => "AS",
        "Anguilla" => "AI",
        "Aruba" => "AW",
        "Bermuda" => "BM",
...
$country_name = extractCountryFromSlug($slug);
    ...
    try {
        ...
        $countryDetails = $database->select(
            ...
            [
                ...
                'visa_countries.visa_type',  // visa_type from visa_countries
                'visa_types.visa_type(visa_type_name)', // visa_type name from visa_types
                'visa_countries.visa_kind', // visa_kind from visa_countries
                'visa_kinds.visa_kind(visa_kind_name)', // visa_kind name from visa_kinds
...
// Fetch visa pricing, including both visa_type and visa_kind
$visa_info = $database->select(
    ...
    [
        ...
        'multiple_entry_fee',
        'visa_on_arrival_fee',
        'visa_types.visa_type(visa_type_name_info)',  // visa_type from visa_pricing
        'visa_kinds.visa_kind(visa_kind_name_info)'   // visa_kind from visa_pricing
    ],
    [
...
// Initialize $seo with an empty string as fallback
$seo = '';

// Only include the schema if it exists
if (isset($schema) && isset($webPageSchema) && isset($breadcrumbSchema) && isset($faqSchema) && isset($productSchema)) {
    $seo = $schema . "\n" . $webPageSchema . "\n" . $breadcrumbSchema . "\n" . $faqSchema . "\n" . $productSchema . "\n" . $metatags;
}
                    ...
                    <div class="carousel-inner rounded-3">
                        <?php foreach ($images as $index => $image): ?>
                            <div class="carousel-item <?= $index === 0 ? 'active' : ''; ?>">
                                <picture>
                                    <source srcset="image.php?image=<?= htmlspecialchars($image['photo_path']); ?>&width=auto&height=700&format=avif&quality=50" type="image/avif">
                                    <source srcset="image.php?image=<?= htmlspecialchars($image['photo_path']); ?>&width=auto&height=700&format=webp&quality=50" type="image/webp">
                                    <img loading="lazy"
                                        src="image.php?image=<?= htmlspecialchars($image['photo_path']); ?>&width=auto&height=700&format=jpeg&quality=50"
...
<section class="container mt-2">
    ...
    <div class="row">
        <div class="col-12">
            <div class="scrolling-wrapper" style="overflow-y: hidden;">
                <!-- Visa Title & Approval Badge (Only Shown Once) -->
                <h1><b><?= htmlspecialchars($country_name); ?> Visa</b></h1>
                <!-- Social Media Buttons (Desktop Only) -->
                <div class="d-none d-lg-block mb-3">
                    <div class="social-buttons d-flex gap-2">
                        <a href="https://www.facebook.com/sharer/sharer.php?u=<?= urlencode((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]") ?>" target="_blank" class="btn btn-sm rounded-pill" style="background-color: #1877F2; color: white;">
                            <i class="bi bi-facebook"></i> Share
                        </a>
                        <a href="https://twitter.com/intent/tweet?url=<?= urlencode((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]") ?>&text=Check out this visa information!" target="_blank" class="btn btn-sm rounded-pill" style="background-color: #000000; color: white;">
                            <i class="bi bi-twitter-x"></i> Share
                        </a>
                        <a href="https://www.linkedin.com/sharing/share-offsite/?url=<?= urlencode((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]") ?>" target="_blank" class="btn btn-sm rounded-pill" style="background-color: #0A66C2; color: white;">
                            <i class="bi bi-linkedin"></i> Share
                        </a>
                        <a href="https://wa.me/?text=Check out this visa information: <?= urlencode((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]") ?>" target="_blank" class="btn btn-sm rounded-pill" style="background-color: #25D366; color: white;">
                            <i class="bi bi-whatsapp"></i> Share
                        </a>
                        <button onclick="copyToClipboard()" class="btn btn-sm rounded-pill" style="background-color: #6c757d; color: white;">
                            <i class="bi bi-link-45deg"></i> Copy Link
                        </button>
                        <a href="mailto:?subject=<?= urlencode(htmlspecialchars($country_name) . ' Visa Information') ?>&body=<?= urlencode('Check out this visa information: ' . (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]") ?>" class="btn btn-sm rounded-pill" style="background-color: #6c757d; color: white;">
                ...
                <script>
                    ...
                    if (navigator.share) {
                        ...
                        shareButton.className = 'btn btn-sm rounded-pill';
                        shareButton.style.backgroundColor = '#007bff';
                        shareButton.style.color = 'white';
                        shareButton.innerHTML = '<i class="bi bi-share"></i> Share via Web';
                        shareButton.onclick = () => {
                            navigator.share({
                                title: '<?= htmlspecialchars($country_name) ?> Visa Information',
                                url: '<?= (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]" ?>'
                            }).catch(console.error);
                        };
                        document.querySelector('.social-buttons').appendChild(shareButton);
                    }
                </script>
                ...
                <!-- <p class="text-muted">
                    <span>
                        <i class="bi bi-file-earmark-text"></i> 96 visa applications processed last month
                    </span>
                    &middot;
                    <span>
    ...
    <div class="row d-flex flex-column-reverse flex-xl-row">
        <div class="col-12 col-xl-8 col-xxl-8">
                ...
                <div class="card visa-card-box">
                    ...
                    <div class="visa-info-grid">
                        ...
                        <div class="visa-info-block">
                            ...
                            <div class="visa-value-text"><?= $visaDetail['visa_type_name'] ?? $info['visa_type_name_info'] ?? 'E-Visa'; ?></div>
                        </div>
                        ...
                        <div class="visa-info-block">
                            ...
                            <div class="visa-value-text">
                                <?php
                                $validities = array_map(fn($info) => $info['visa_validity'], $visa_info);
                                $uniqueValidities = array_unique($validities);
                                $count = count($uniqueValidities);
                                $formattedValidities = array_map(function ($validity) {
                                    return $validity >= 365 ? floor($validity / 365) . " years" : $validity . " days";
                                }, $uniqueValidities);
                                if ($count > 2) {
                                    echo implode(', ', array_slice($formattedValidities, 0, -1)) . ' or ' . end($formattedValidities);
                                } elseif ($count === 2) {
                                    echo implode(' or ', $formattedValidities);
                                } elseif ($count === 1) {
                                    echo $formattedValidities[0];
                                }
                                ?>
                            </div>
                            <div class="visa-description-text">
            ...
            <!-- Note -->
            <?php if (!empty($countryData['extra_details'])): ?>
                <h2 class="heading-underline fw-bold mt-3">Note</h2>
            ...
            <?php if (!empty($faqs)): ?>
                <div class="accordion" id="faqAccordion">
                    <?php foreach ($faqs as $index => $faq): ?>
                        <div class="accordion-item" style="border: 1px solid var(--blue); <?= ($index !== array_key_last($faqs)) ? 'border-bottom: none;' : ''; ?>">
            ...
            <!-- Visa Rejections -->
            <?php include 'components/visa-rejections.php'; ?>
            <!--<div>
        ...
        <div class="col-12 col-xl-4 col-xxl-4 mb-3">
            ...
            <!-- visa details -->
            <h2 class="heading-underline fw-bold mt-3 widget js-sticky-widget">Available Visa Options</h2>


            <?php foreach ($visa_info as $info): ?>
                <div class="card mb-2 bg-gradient widget js-sticky-widget" style="border: 1px solid var(--blue);">
                    <div class="card-header text-white" style="background-color: var(--blue); border-bottom: 1px solid var(--blue);">
                        <b><?= $visaDetail['country_name']; ?> <?= $visaDetail['visa_kind_name'] ?? $info['visa_kind_name_info'] ?? 'Not available' ?></b>
                        <p class="card-text fw-bold mb-1"><?= $info['pricing_name']; ?></p>
                        <?php if ($countryData['is_this_schengen'] == 1): ?>
                            <span class="badge bg-secondary rounded-pill">Schengen Visa</span>
                        <?php endif; ?>
                        <p class="card-text mt-2">
                            <?php if ($countryData['is_this_schengen'] === 1 || $country_name === 'Philippines'): ?>
                                <button type="button" class="btn rounded-pill bg-gradient" style="background-color: var(--golden-light);" data-bs-toggle="modal" data-bs-target="#contactUsModal">Contact Us <i class="bi bi-arrow-up-right-circle"></i></button>
                            <?php else: ?>
                                <a href="start-application?pid=<?= $info['id']; ?>&cid=<?= $countryData['country_id']; ?>" class="btn rounded-pill bg-gradient" style="background-color: var(--golden-light);">Start Application <i class="bi bi-arrow-up-right-circle"></i></a>
                            <?php endif; ?>
                        </p>
                    </div>
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item" style="border-bottom: 1px solid var(--blue);">Visa Type <span class="float-end text-muted"><?= $visaDetail['visa_type_name'] ?? $info['visa_type_name_info'] ?? 'Not available'; ?></span></li>
                        ...
                        <li class="list-group-item" style="border-bottom: 1px solid var(--blue);">Visa Validity <span class="float-end text-muted"><?= $info['visa_validity'] >= 365 ? floor($info['visa_validity'] / 365) . ' Year' . (floor($info['visa_validity'] / 365) > 1 ? 's' : '') : $info['visa_validity'] . ' Days'; ?></span></li>
                        ...
                        <li class="list-group-item fees-modal-trigger">
                            <b>Total Amout</b> <span class="float-end text-golden">
            ...
            <div class="modal-body">
                ...
                <form id="contactForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control form-control-lg" id="country_name" name="country_name" value="<?= $countryData['country_name']; ?>" readonly>
                    </div>
                    <div class="mb-3">
                    ...
                    <input type="hidden" id="userCountryCode" name="userCountryCode">
                    <button type="button" class="btn btn-golden btn-lg w-100 mt-3" id="submitContactForm">Submit</button>
                </form>
            </div>
        </div>
    </div>
...
<!-- Nationality Restriction Modal -->
<div class="modal fade" id="nationalityRestrictionModal" tabindex="-1" aria-labelledby="nationalityRestrictionModalLabel" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header text-white" style="background-color: var(--blue);">
                <h5 class="modal-title" id="nationalityRestrictionModalLabel">Nationality Restrictions</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <p class="mb-4">Currently, we only support visa applications for Singapore from the following nationalities:</p>
                <div class="row">
                    <div class="col-md-6">
                        <ul class="list-group">
                            <li class="list-group-item"><?= countryToEmoji('Algeria'); ?> Algeria</li>
                            <li class="list-group-item"><?= countryToEmoji('Armenia'); ?> Armenia</li>
                            <li class="list-group-item"><?= countryToEmoji('Azerbaijan'); ?> Azerbaijan</li>
                            <li class="list-group-item"><?= countryToEmoji('Belarus'); ?> Belarus</li>
                            <li class="list-group-item"><?= countryToEmoji('Democratic People\'s Republic of Korea'); ?> Democratic People's Republic of Korea</li>
                            <li class="list-group-item"><?= countryToEmoji('Egypt'); ?> Egypt</li>
                            <li class="list-group-item"><?= countryToEmoji('Georgia'); ?> Georgia</li>
                            <li class="list-group-item"><?= countryToEmoji('Iran'); ?> Iran</li>
                            <li class="list-group-item"><?= countryToEmoji('Iraq'); ?> Iraq</li>
                            <li class="list-group-item"><?= countryToEmoji('Jordan'); ?> Jordan</li>
                            <li class="list-group-item"><?= countryToEmoji('Kazakhstan'); ?> Kazakhstan</li>
                            <li class="list-group-item"><?= countryToEmoji('Kosovo'); ?> Kosovo</li>
                            <li class="list-group-item"><?= countryToEmoji('Kyrgyzstan'); ?> Kyrgyzstan</li>
                            <li class="list-group-item"><?= countryToEmoji('Lebanon'); ?> Lebanon</li>
                            <li class="list-group-item"><?= countryToEmoji('Libya'); ?> Libya</li>
                        </ul>
                    </div>
                    <div class="col-md-6">
                        <ul class="list-group">
                            <li class="list-group-item"><?= countryToEmoji('Mali'); ?> Mali</li>
                            <li class="list-group-item"><?= countryToEmoji('Moldova'); ?> Moldova</li>
                            <li class="list-group-item"><?= countryToEmoji('Morocco'); ?> Morocco</li>
                            <li class="list-group-item"><?= countryToEmoji('Pakistan'); ?> Pakistan</li>
                            <li class="list-group-item"><?= countryToEmoji('Russia'); ?> Russia</li>
                            <li class="list-group-item"><?= countryToEmoji('South Sudan'); ?> South Sudan</li>
                            <li class="list-group-item"><?= countryToEmoji('Sudan'); ?> Sudan</li>
                            <li class="list-group-item"><?= countryToEmoji('Syria'); ?> Syria</li>
                            <li class="list-group-item"><?= countryToEmoji('Tunisia'); ?> Tunisia</li>
                            <li class="list-group-item"><?= countryToEmoji('Turkmenistan'); ?> Turkmenistan</li>
                            <li class="list-group-item"><?= countryToEmoji('Ukraine'); ?> Ukraine</li>
                            <li class="list-group-item"><?= countryToEmoji('Yemen'); ?> Yemen</li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-blue rounded-pill" data-bs-dismiss="modal">Proceed</button>
            </div>
        </div>
    </div>
</div>
...
<!-- Script to Handle Date Restrictions -->
<!-- <script>
...
<script>
    ...
    // Hide sticky widget when footer is in view
    const footer = document.querySelector('footer');
    const widget = document.querySelector('.widget.js-sticky-widget');
...
<script>
    // Check if country is Singapore and show nationality restriction modal
    document.addEventListener('DOMContentLoaded', function() {
        const countryName = '<?= $country_name; ?>'.toLowerCase();
        if (countryName === 'singapore') {
            // Check if modal has been shown in this session
            if (!localStorage.getItem('nationalityModalShown')) {
                const nationalityModal = new bootstrap.Modal(document.getElementById('nationalityRestrictionModal'));
                nationalityModal.show();
                // Set flag in localStorage
                localStorage.setItem('nationalityModalShown', 'true');
            }

            // Add click event listener to all Start Application buttons
            document.querySelectorAll('a[href^="start-application"]').forEach(button => {
                button.addEventListener('click', function(e) {
                    e.preventDefault(); // Prevent default link behavior
                    const nationalityModal = new bootstrap.Modal(document.getElementById('nationalityRestrictionModal'));
                    nationalityModal.show();

                    // When Proceed is clicked, navigate to the application page
                    document.querySelector('#nationalityRestrictionModal .btn-blue').addEventListener('click', function() {
...
<script src="assets/js/schengen.js"></script>
